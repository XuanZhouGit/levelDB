...
![InternalKey](https://github.com/XuanZhouGit/levelDB/blob/master/levelDB3.PNG)
其中:
...
![entry](https://github.com/XuanZhouGit/levelDB/blob/master/levelDB4.PNG)
## 3.3 .sst file(data block)
.sst file的整体结构如图所示：
![.sst file](https://github.com/XuanZhouGit/levelDB/blob/master/levelDB5.PNG)
...
Footer是文件的结尾,其结构为:
![Footer](https://github.com/XuanZhouGit/levelDB/blob/master/levelDB6.PNG)
...
![TableBuilder](https://github.com/XuanZhouGit/levelDB/blob/master/levelDB7.PNG)
其中type表示是否进行压缩

而data_block, index_block以及metaindex_block的具体结构为：
![metaindex_block](https://github.com/XuanZhouGit/levelDB/blob/master/levelDB8.PNG)
...
每条entry的结构为
![entry](https://github.com/XuanZhouGit/levelDB/blob/master/levelDB9.PNG)
...
![file_reader](https://github.com/XuanZhouGit/levelDB/blob/master/levelDB10.PNG)
...
Options options;
Status status;
RandomAccessFile* file; // sst file
uint64_t cache_id; // table cache finding
...
![file writer](https://github.com/XuanZhouGit/levelDB/blob/master/levelDB11.PNG)
tablebuilder::rep的结构为：
```
struct TableBuilder::Rep {
Options options;
Options index_block_options;
WritableFile* file; // .sst file handle
uint64_t offset; // current write offset
Status status;
BlockBuilder data_block;
BlockBuilder index_block;
std::string last_key; // current last key
int64_t num_entries; // total entries number in file
...
std::string compressed_output; // tmp string, use to store compressed_result
};
```
...
![meta block](https://github.com/XuanZhouGit/levelDB/blob/master/levelDB12.PNG)
 filter clock的结构为：
![filter block](https://github.com/XuanZhouGit/levelDB/blob/master/levelDB13.PNG)
...
![filter index](https://github.com/XuanZhouGit/levelDB/blob/master/levelDB14.PNG)
...
log writer的结构如图所示：
![log writer](https://github.com/XuanZhouGit/levelDB/blob/master/levelDB15.PNG)
log reader的结构如图所示：
![log reader](https://github.com/XuanZhouGit/levelDB/blob/master/levelDB16.PNG)
### 3.4.2 文件结构
log file由若干大小为32KB的block组成，每个block由若干record组成，每条record的结构为：
![record](https://github.com/XuanZhouGit/levelDB/blob/master/levelDB17.PNG)
...
一个例子：
![log file](https://github.com/XuanZhouGit/levelDB/blob/master/levelDB17.PNG)
## 3.5 manifest file(log file)
每个manifest file会管理这个version所有的信息，其内容结构为：
![manifest file](https://github.com/XuanZhouGit/levelDB/blob/master/levelDB18.PNG)
current file保存了当前manifest file的名字
# 4 levelDB的key-value读写流程
## 4.1 写流程
levelDB的写速度相对于读来说速度很快，也比较简单。首先顺序写log文件然后将key-value插入skiplist, 其流程为：
![skiplist](https://github.com/XuanZhouGit/levelDB/blob/master/levelDB19.PNG)
在levelDB中，delete也是作为write_record被写进levelDB的
![write_record](https://github.com/XuanZhouGit/levelDB/blob/master/levelDB20.PNG)
## 4.2 读流程
...
读流程如图所示：
![read flow](https://github.com/XuanZhouGit/levelDB/blob/master/levelDB21.PNG)
其中：
...
(2) 当level1~N中的levelK的data size超过(10^K)MB，就会将levelK中的一个文件和level(K+1)中的所有文件merge生成新的level(K+1)
(3) 当level的查询次数到达阀值时，该level就要进行compaction
而compaction range则是由上次该level的compaction range的最后一个key的下一条开始的(compaction比较均匀)，通过compaction, data逐渐从低层向高层移动，以减少查找及遍历开销
compaction的总流程图为：
![compaction](https://github.com/XuanZhouGit/levelDB/blob/master/levelDB22.PNG)
...
![merge flow](https://github.com/XuanZhouGit/levelDB/blob/master/levelDB23.PNG)
# 6 Iterator
Iterator是levelDB中所有查找、遍历以及归并的执行者。这些iterator的关系为：
![iterator](https://github.com/XuanZhouGit/levelDB/blob/master/levelDB24.PNG)
其中:
(1)TwoLevelIterator是用于遍历每个sst文件iter,其中包含一个index_iter_以及data_iter_, data_iter_初始是空的，在seek到index_iter_时，调用InitDataBlock时才会被初始化。
